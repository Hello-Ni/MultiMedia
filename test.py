...
gaussian_component = [2, 5, 8, 11]


for i in range(len(gaussian_component)):
    n = gaussian_component[i]
    gmm_model = GMM(
        n_components=n, covariance_type='tied').fit(img1_resize)
    ...
    mask_1 = (segmented1)*255  # black 0 white 255 scene
    scene1 = plt.subplot(2, 4, i*2)
    scene1.title.set_text(s1.s1_acc)
    plt.imshow(mask_1, cmap=plt.cm.gray)
    plt.axis('off')
    ...
    mask_2 = (segmented2)*255  # black 0 white 255 scene
    scene2 = plt.subplot(2, 4, i*2+1)
    scene1.title.set_text(s2_acc)
    plt.imshow(mask_2, cmap=plt.cm.gray)
    plt.axis('off')
plt.show()
